...
#ifndef HOG2_SPL_PEMM_H
#define HOG2_SPL_PEMM_H
...
namespace EMBHS { namespace PEMM {
    ...
    class PEMM {
    public:
        PEMM(std::string cPrefix = "PEMM_files/", EMBHS::Verbosity cVerbosity = EMBHS::lInfo) :
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::GetPath(Tstate &cStart, Tstate &cGoal, uint8_t eps) {
        this->start = cStart;
        this->goal = cGoal;
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::SetHeuristics(Heuristic<Tstate> *hF, Heuristic<Tstate> *hB) {
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void
    PEMM<Tstate, Taction, Tenvironment, Thash>::AddStateToQueue(const Tstate &state, tSearchDirection direction,
                                                                uint8_t gCost) {
        OpenData openData{};
        DiskState diskState{};
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::GetOpenAndDiskData(const Tstate &state, tSearchDirection dir,
        ...
        if (openData.gCost < openData.hBarCost) {
            printLock.lock();
            std::cout << "Error: " << state << " g:" << unsigned(openData.gCost) << " hb: "
                      << unsigned(openData.hBarCost)
                      << std::endl;
            printLock.unlock();
        }
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void
    PEMM<Tstate, Taction, Tenvironment, Thash>::AddStatesToQueue(const OpenData &openData, DiskState *diskState,
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    std::string
    PEMM<Tstate, Taction, Tenvironment, Thash>::GetOpenName(const OpenData &openData, tSearchDirection dir) {
        std::string s;
        s += this->prefix;
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::ExpandNextFile() {
        ...
        if (verbosity <= EMBHS::lInfo) {
            printLock.lock();
            std::cout << "Next: [" << currExpansionDir << "] " << openData << " (" << states.size() << " entries ";
        ...
        openLock.lock();
        std::thread t(&PEMM::CheckSolution, this, currExpansionDir, openData, std::ref(states));
        openLock.unlock();
        ...
        for (size_t x = 0; x < numThreads; x++)
            threads.push_back(
                    new std::thread(&PEMM::ParallelExpandBucket, this, openData, std::ref(states),
                                    currExpansionDir, x,
                                    numThreads));
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    OpenData PEMM<Tstate, Taction, Tenvironment, Thash>::GetBestFile() {

        auto &forwardQueue = GetOpenQueue(kForward);
        OpenData best = (forwardQueue.begin())->first;
        currExpansionDir = kForward;
        uint16_t bestPriority = std::max(((uint16_t) best.gCost) + best.hCost, ((uint16_t) 2) * best.gCost);
        uint16_t currPriority;
        for (const auto &s: forwardQueue) {
            currPriority = std::max(((uint16_t) 2) * s.first.gCost, ((uint16_t) s.first.gCost + s.first.hCost));
            // Always prefer lower priority, and tiebreak in favor of lower g-values. This promises we always expand
        ...
        auto &backwardQueue = GetOpenQueue(kBackward);
        for (const auto &s: backwardQueue) {
            currPriority = std::max(((uint16_t) 2) * s.first.gCost, ((uint16_t) s.first.gCost + s.first.hCost));
            // Always prefer lower priority, and tiebreak in favor of lower g-values. This promises we always expand
            //  the parent before the child
            if (currPriority < bestPriority || (currPriority == bestPriority && s.first.gCost < best.gCost) ||
...
// This method collects all the minimal values (g,f,d,b) that we need to bound calculations.
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    BoundValues PEMM<Tstate, Taction, Tenvironment, Thash>::GetBoundValues(tSearchDirection dir) {
        auto &openQueue = GetOpenQueue(dir);
        BoundValues bv{UINT16_MAX, UINT16_MAX};
        for (const auto &s: openQueue) {
            if (s.first.gCost < bv.minG)
                bv.minG = s.first.gCost;
            uint16_t currentPr = std::max(((uint16_t) s.first.gCost) + s.first.hCost, ((uint16_t) 2) * s.first.gCost);
            if (currentPr < bv.minPr)
                bv.minPr = currentPr;
        }
        return bv;
    }
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    bool PEMM<Tstate, Taction, Tenvironment, Thash>::CanTerminateSearch() {
        BoundValues forwardBV = GetBoundValues(kForward);
        BoundValues backwardBV = GetBoundValues(kBackward);
        if (verbosity <= EMBHS::lDebug) {
            printLock.lock();
            std::cout << forwardBV << std::endl;
            std::cout << backwardBV << std::endl;
            printLock.unlock();
        }
        uint16_t gBound = forwardBV.minG + backwardBV.minG;
        uint16_t prBound = std::min(forwardBV.minPr, backwardBV.minPr);
        lb = std::max(gBound, prBound);
        if (verbosity <= EMBHS::lInfo) {
            std::cout << "S: ";
            ...
            std::cout << " B: " << unsigned(gBound) << "," << unsigned(prBound) << "\n";
        }
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::ReadAndDDBucket(BucketSet &states, const OpenData &openData,
                                                                     tSearchDirection dir) {
...
// get only a single copy of a state even if it appeared multiple times in the file.
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::ReadBucket(BucketSet &states, const OpenData &openData,
                                                                tSearchDirection dir) {
        auto &openQueue = GetOpenQueue(dir);
        states.resize(openQueue[openData].writtenStates);
        //const auto numThreads = 1;
        std::vector<std::thread *> threads;
        const auto numReadingThreads = std::thread::hardware_concurrency();
        for (size_t x = 0; x < numReadingThreads; x++)
            threads.push_back(new std::thread(&PEMM::ReadBucketThread, this, std::ref(states), openData, dir, x,
                                              numReadingThreads));
        for (auto &thread: threads) {
            thread->join();
            delete thread;
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::ReadBucketThread(BucketSet &states, const OpenData &openData,
...
// g-values. Since we work in unit edge cost domains (for now), we only need to check 2 backwards.
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::RemoveDuplicates(BucketSet &states, const OpenData &openData,
                                                                      tSearchDirection dir) {
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::WriteToClosed(BucketSet &states, const OpenData &openData,
                                                                   tSearchDirection dir) {
        ClosedData closedData{};
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    std::string PEMM<Tstate, Taction, Tenvironment, Thash>::GetClosedName(const ClosedData &closedData) {
        std::string s;
        s += this->prefix;
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void PEMM<Tstate, Taction, Tenvironment, Thash>::CheckSolution(tSearchDirection currDir, OpenData openData,
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void
    PEMM<Tstate, Taction, Tenvironment, Thash>::FindSolution(const OpenData &openData, const OpenFile &openFile,
    ...
    template<class Tstate, class Taction, class Tenvironment, class Thash>
    void
    PEMM<Tstate, Taction, Tenvironment, Thash>::ParallelExpandBucket(const OpenData &openData, BucketSet &states,
                                                                     tSearchDirection dir, int myThread,
                                                                     int totalThreads) {
