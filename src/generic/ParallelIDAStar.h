...
public:
	...
	void GetPath(environment *env, state from, state to,
				 std::vector<action> &thePath, uint64_t numThreads = std::thread::hardware_concurrency());

	uint64_t GetNodesExpanded() { return nodesExpanded; }
    uint64_t GetNecessaryExpanded() { return nodesNecessaryExpanded; }
	uint64_t GetNodesTouched() { return nodesTouched; }
	void ResetNodeCount() { nodesExpanded = nodesTouched = 0; }
	void SetHeuristic(Heuristic<state> *heur) { heuristic = heur; if (heur != 0) storedHeuristic = true;}
	void SetStopAfterSolution(bool fas) { finishAfterSolution=fas;}
private:
    uint64_t nodesExpanded, nodesTouched, nodesNecessaryExpanded;

	void StartThreadedIteration(environment env, state startState, double bound, int numThread);
    ...
    std::vector<uint64_t> threadNodesExpanded;
    std::vector<uint64_t> threadNodesTouched;
};
...
void ParallelIDAStar<environment, state, action>::GetPath(environment *env,
                                                          ...
                                                          uint64_t numThreads)
{
    threadNodesExpanded.resize(numThreads);
    threadNodesTouched.resize(numThreads);
    ...
    do {
        ...
        nodesTouched = 0;
        nodesExpanded = 0;
        nodesNecessaryExpanded = 0;
        GenerateWork(env, act[0], from, thePath);
    } while (work.size()<numThreads*2);
	GenerateWork(env, act[0], from, thePath);
	...
	while (foundSolution > work.size())
	{
		...
		fflush(stdout);
        uint64_t previouslyExpended = nodesExpanded;
		...
		{
			threads[x]->join();
            std::cout << "Thread " << x <<": Time: " << threadTimes[x] << "s; Expanded: " << threadNodesExpanded[x] <<
            "; Generated: " << threadNodesTouched[x] << "\n";
			delete threads[x];
			threads[x] = 0;
		}
...
//				printf("<<Solution histogram<<\n");
		...
		nextBound = bestBound;
        nodesNecessaryExpanded = nodesExpanded - previouslyExpended;
...
template <class environment, class state, class action>
void ParallelIDAStar<environment, state, action>::StartThreadedIteration(environment env, state startState, double bound, int numThread)
{
	vectorCache<action> actCache;
	std::vector<action> thePath;
    Timer t;
    threadNodesExpanded[numThread]=0;
    threadNodesTouched[numThread]=0;
    t.StartTimer();
	while (true)
	{
		int nextValue;
        ...
        threadNodesExpanded[numThread] += localWork.expanded;
        threadNodesTouched[numThread] += localWork.touched;
	}
    t.EndTimer();
    threadTimes[numThread] = t.GetElapsedTime();
}
